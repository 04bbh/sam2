...
class QwenVLDetector:
    ...
    def _build_prompt(self, batch_size: int, target_desc: str | None = None) -> str:
        ...
        # Task
        给定 {batch_size} 张图像，请分别在每张图像中检测"Detection Categories"中定义的目标，
        并输出对应目标的"category"、"box"和"confidence"。
        若图像中不存在任何定义的目标，则输出空数组 []。
        ...
        * 必须包含 image_id 从 0 到 {batch_size}-1 的所有项
        ...
        # """
        # - 0.90-1.00：目标类别明确，关键视觉证据清晰完整，box 紧密覆盖完整目标。
        # - 0.75-0.89：目标类别明确，box 基本准确，但存在轻微遮挡、模糊、边界偏差或背景冗余。
        # - 0.50-0.74：目标疑似存在，但关键证据不足，或 box 不够准确。
        # - 0.40-0.49：低置信疑似目标，仅当目标确实与类别高度相关时输出。
        # - <0.40：不要输出。

        # 若类别不确定或 box 无法可靠确定，优先输出 []，不要猜测。
